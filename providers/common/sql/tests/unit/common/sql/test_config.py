...
class TestDataSourceConfig:
    ...
    def test_explicit_storage_type_without_uri_raises_error(self):
        with pytest.raises(ValueError, match="URI must be provided when storage_type is set"):
            DataSourceConfig(conn_id="pg", table_name="my_table", storage_type=StorageType.S3)

    def test_explicit_storage_type_without_uri_raises_error_with_format(self):
        with pytest.raises(ValueError, match="URI must be provided when storage_type is set"):
            DataSourceConfig(
